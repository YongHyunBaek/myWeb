...
public Vector<ZipCodeVO> zipcodeRead(String dong){
	...
	public int loginCheck(String id, String pass) {
		...
		try {
			...
			if(rs != null)try {rs.close();}catch(SQLException sql1) {}
			if(pstmt != null)try {pstmt.close();}catch(SQLException sql2) {}
			if(conn != null)try {conn.close();}catch(SQLException sql3) {}
		}
		return check;
	}
